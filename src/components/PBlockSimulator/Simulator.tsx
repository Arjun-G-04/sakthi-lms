...
import { Instructions } from "#/components/TcsIonSimulator/Instructions";
import { SubmitModal } from "#/components/TcsIonSimulator/SubmitModal";
import { addTestPerformance } from "#/lib/test-performance.functions";
import { MathText } from "./MathText";
import { pBlockQuestions } from "./questions";
...
export function Simulator() {
	...
	const [status, setStatus] = useState<Record<number, QuestionStatus>>(() => {
		...
		for (const q of pBlockQuestions) {
	...
	const currentQuestion = pBlockQuestions[currentIdx];
	...
	const selectQuestion = useCallback((idx: number) => {
		setCurrentIdx(idx);
		setStatus((prev) => {
			const qId = pBlockQuestions[idx].id;
	...
	const handleSaveNext = useCallback(() => {
		...
		if (currentIdx < pBlockQuestions.length - 1) {
	...
	return (
		<div className="flex flex-col gap-5 min-h-[calc(100vh-100px)] text-[#1a2840] animate-fade-in relative z-10">
			...
			<div className="flex flex-col gap-3 rounded-2xl border border-[#1a2840]/12 bg-[#2d5a3d]/90 p-4 text-[#fdfaf4] shadow-md sm:flex-row sm:items-center sm:justify-between">
				<div>
					<h2 className="display-title text-2xl font-bold">p-block</h2>
				</div>
			...
			{showSubmitModal && (
				<SubmitModal
					...
					totalCount={pBlockQuestions.length}
				/>
			)}
		</div>
	);
